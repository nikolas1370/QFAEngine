...
shaderc_compiler_t QFAShaderCompiler::Compiler;
QFAShaderCompiler::ShaderListStart QFAShaderCompiler::DefaultList;
std::vector<shaderc_compilation_result_t> QFAShaderCompiler::ShadercCompilationResult;
...
void QFAShaderCompiler::ProcessShaders(const std::u32string& shaderFilderPath)
{
    if (!QFAFileSystem::exists(shaderFilderPath))
        stopExecute("shaderFilderPath not exist")

    const std::u32string binPath = shaderFilderPath + U"/spv/";
    if (!QFAFileSystem::exists(binPath))
        QFAFileSystem::CreateFolders(binPath);

    ShaderListStart* sl = nullptr;
    void* shaders = nullptr;

    QFAFile file;
    std::filesystem::path shaderListPath = std::filesystem::path(shaderFilderPath).append(U"ShaderList.qfa");
    ...
    if (QFAFileSystem::GetFileList(shaderFilderPath, filePaths))
    {
        for (size_t i = 0; i < filePaths.size(); i++)
        {
            ...
            compFileName = std::u32string(binPath).append(compFileName.filename().u32string());
            if (needCompile)
            {
            ...
            else
            {
                if (QFAFileSystem::exists(compFileName.u32string()))
                {
                    ...
                    QFAVKPipeline::ShaderData.push_back(sh);
                }
                else
                {
                    Compile(filePaths[i], kind, indexInList, sp, compFileName);
